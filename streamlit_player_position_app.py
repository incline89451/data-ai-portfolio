...
def build_dataset(tables: Dict[str, pd.DataFrame]) -> Tuple[pd.DataFrame, pd.Series, pd.DataFrame]:
    ...
    feats = app_eligible.merge(people, on="playerID", how="left").merge(fld_agg, on="playerID", how="left")
...
st.caption("Classify players into their most effective positions using fielding stats and physical attributes.")
...
with col1:
    ...
    st.write(f"**Overall Accuracy:** {acc:.3f}")
    
    st.subheader("Confusion Matrix")
    st.caption("Shows correct and incorrect predictions per position. Diagonal = correct predictions.")
